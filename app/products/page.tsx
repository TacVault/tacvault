  ...
  return (
    <main className="min-h-screen bg-[#08090a] text-[#e8eaed]">
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <p className="text-[#8b939c] mb-8">7 carriers compared</p>
        <div className="space-y-4 mb-16">
          {carriers.map((carrier) => (
            <a key={carrier.href} href={carrier.href} className="block bg-[#121416] border border-[#1e2124] rounded-lg p-4 hover:border-[#3a3d42] transition-colors">
        ...
        <h1 className="font-rajdhani font-bold text-4xl mb-2">Placards and Chest Rigs</h1>
        <p className="text-[#8b939c] mb-8">Modular front panels</p>
        <div className="space-y-4">
          {placards.map((placard) => (
            <a key={placard.href} href={placard.href} className="block bg-[#121416] border border-[#1e2124] rounded-lg p-4 hover:border-[#3a3d42] transition-colors">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-xs text-[#a8b2bc] border border-[#2a2d31] rounded px-2 py-0.5 mb-2 inline-block">{placard.badge}</span>
                  <h2 className="font-rajdhani font-bold text-xl text-[#e8eaed]">{placard.name}</h2>
                </div>
                <span className="font-rajdhani text-xl text-[#a8b2bc]">{placard.price}</span>
              </div>
            </a>
          ))}
        </div>
